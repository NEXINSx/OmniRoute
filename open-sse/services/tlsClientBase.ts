/**
 * Shared browser-impersonating HTTP transport for the six web-cookie providers.
 *
 ...
 *
 * Every wreq request uses an ephemeral cookie scope. Transports are reused only
 * within one provider module and are keyed by browser profile, emulated OS, and
 * resolved proxy; no cookie jar or session identifier is shared between calls.
 */
...
interface WreqTransportLike {
  close: () => Promise<void> | void;
}

export interface WreqRuntimeLike {
  createTransport: (options: Record<string, unknown>) => Promise<WreqTransportLike>;
  fetch: (url: string, options: Record<string, unknown>) => Promise<TlsResponseLike>;
}

export type WreqRuntimeLoader = () => Promise<WreqRuntimeLike>;
    ...
    async pull(controller) {
      while (!closed) {
        ...
        if (eofReached) {
          ...
          closed = true;
          cleanupLifecycle();
          await reader.cancel("TLS stream EOF reached").catch(() => {});
          controller.close();
          return;
        }
...
async function loadWreqRuntime(): Promise<WreqRuntimeLike> {
  const moduleName = "wreq-js";
  const loaded = Reflect.apply(runtimeRequire, undefined, [moduleName]) as Partial<WreqRuntimeLike>;
  ...
  return loaded as WreqRuntimeLike;
}

export interface TlsRequestClient {
  request: (url: string, options: Record<string, unknown>) => Promise<TlsResponseLike>;
  invalidateTransport?: (options: Record<string, unknown>) => void;
}

function transportKey(browser: string, os: EmulationOs, options: Record<string, unknown>): string {
  const proxy = typeof options.proxyUrl === "string" ? options.proxyUrl : "";
  return `${browser}\0${os}\0${proxy}`;
}

/** Create a lazy wreq-js client with transport-only pooling and no shared cookie jar. */
export function createGetClient(config: {
  ...
  const transports = new Map<string, Promise<WreqTransportLike>>();
  let runtimePromise: Promise<WreqRuntimeLike> | null = null;

  const getRuntime = (): Promise<WreqRuntimeLike> => {
    if (!runtimePromise) {
      const pending = (config.wreqRuntimeLoader ?? loadWreqRuntime)().catch(() => {
        if (runtimePromise === pending) runtimePromise = null;
        throw new TlsClientUnavailableError(
          `wreq-js 3.0.0 is not installed or unsupported on this platform — ` +
  ...
  const closeTransport = (key: string): void => {
    const pending = transports.get(key);
    if (!pending) return;
  ...
  transportClosers.add(closeAllTransports);
  if (!exitHookInstalled) {
    exitHookInstalled = true;
    process.once("exit", () => {
      for (const close of transportClosers) close();
      transportClosers.clear();
    });
  }
  ...
  const client: TlsRequestClient = {
    async request(url, options) {
      ...
      const key = transportKey(browser, os, options);
      ...
      if (!pending) {
        ...
        pending = runtime.createTransport(transportOptions);
    ...
    invalidateTransport(options) {
      closeTransport(transportKey(browser, os, options));
    },
  };
...
export function createTlsClientModule(config: TlsClientConfig): TlsClientModule {
  ...
  const invalidateOnHang = (
    ...
    if (error instanceof TlsClientHangError) client.invalidateTransport?.(requestOptions);
  };
  ...
  async function tlsFetchStreaming(
    ...
    try {
      ...
      if (responseValidation === "cf" && isCloudflareChallenge(preview)) {
        await activeReader.cancel("Cloudflare challenge").catch(() => {});
        return {
          status: 403,
      ...
      if (responseValidation === "cf" && preview.trimStart().startsWith("<")) {
        await activeReader.cancel("HTML response").catch(() => {});
        return {
          status: 502,
          headers: new Headers({ "Content-Type": "text/html" }),
          text: preview,
      ...
      const stream = createEofFilteredStream(
        ...
        (error) => invalidateOnHang(client, requestOptions, error),
        signal,
        hardDeadlineAt
      );
      reader = null;
      ...
      invalidateOnHang(client, requestOptions, error);
